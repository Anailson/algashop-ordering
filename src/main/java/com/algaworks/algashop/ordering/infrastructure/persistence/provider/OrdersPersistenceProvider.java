...
public class OrdersPersistenceProvider implements Orders {
    ...
    public void add(Order aggregateRoot) {
        ...
        persistenceRepository.findById(orderId)
                .ifPresentOrElse(
                        (persistenceRepository) -> update(aggregateRoot, persistenceRepository),
                        () -> insert(aggregateRoot)
                );
    }

    @Override
    public List<Order> placeByCustomerInYEar(CustomerId customerId, Year year) {
        OffsetDateTime start = year.atDay(1).atStartOfDay().atOffset(ZoneOffset.UTC);
        OffsetDateTime end = start.plusYears(1).minusNanos(1);

        List<OrderPersistenceEntity> entities = persistenceRepository.findByCustomer_IdAdndPlaceatBetween(
                customerId.value(),
                start,
                end
        );
