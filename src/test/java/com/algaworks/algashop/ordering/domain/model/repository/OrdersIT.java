...
class OrdersIT {
    ...
    public void shouldListExistingOrdersByYear() {
        ...
        List<Order> listedOrders = orders.placeByCustomerInYEar(customerId, Year.now());
        Assertions.assertThat(listedOrders).isNotEmpty();
        Assertions.assertThat(listedOrders.size()).isEqualTo(2);

        listedOrders = orders.placeByCustomerInYEar(customerId, Year.now().minusYears(1));
        Assertions.assertThat(listedOrders).isEmpty();

        listedOrders = orders.placeByCustomerInYEar(new CustomerId(), Year.now());
        Assertions.assertThat(listedOrders).isEmpty();
